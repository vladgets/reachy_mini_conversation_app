...
from reachy_mini_conversation_app_vlad.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from reachy_mini_conversation_app_vlad.config import config  # noqa: F401
from reachy_mini_conversation_app_vlad.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from reachy_mini_conversation_app_vlad.tools.background_tool_manager import BackgroundToolManager
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"reachy_mini_conversation_app_vlad.tools.{tool_name}"
            try:
