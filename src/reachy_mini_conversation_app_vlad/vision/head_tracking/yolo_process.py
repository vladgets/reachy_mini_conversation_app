...
from reachy_mini_conversation_app_vlad.vision.head_tracking import HeadTracker, HeadTrackerResult
...
def _build_tracker_backend() -> HeadTracker:
    ...
    from reachy_mini_conversation_app_vlad.vision.head_tracking.yolo import YoloHeadTracker
...
class YoloHeadTrackerProcess:
    ...
    def __init__(self, *, request_timeout: float = _REQUEST_TIMEOUT) -> None:
        ...
        module_path = "reachy_mini_conversation_app_vlad.vision.head_tracking.yolo_process"
        env = os.environ.copy()
        package_spec = importlib.util.find_spec("reachy_mini_conversation_app_vlad")
        package_locations = None if package_spec is None else package_spec.submodule_search_locations
        if not package_locations:
            raise RuntimeError("Unable to determine the reachy_mini_conversation_app_vlad package path")
...
def main() -> int:
    ...
    if len(sys.argv) != 1:
        print(
            "usage: python -m reachy_mini_conversation_app_vlad.vision.head_tracking.yolo_process",
            file=sys.stderr,
        )
        return 2
    return _worker_main()
